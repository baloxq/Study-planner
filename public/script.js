...
/* ============================================================
   API #1 – STATIC API: Inspirational quotes
   Fetches from the free, open-source Dummyjson quotes endpoint
   (no key, no proxy, reliable CORS). Falls back to a local pool
   so the card always displays something meaningful.
...
const FALLBACK_QUOTES = [
  ...
  { q: "Study hard what interests you the most in the most undisciplined, irreverent way possible.", a: "Richard P. Feynman" },
  { q: "Learning is not attained by chance. It must be sought for with ardor and attended to with diligence.", a: "Abigail Adams" },
...
function showLocalQuote() {
  ...
  box.innerHTML = `
    <div class="quote-text">"${pick.q}"</div>
    <div class="quote-author">— ${pick.a}</div>
  `;
}
...
async function loadQuote() {
  ...
  try {
    // dummyjson.com/quotes — free, open CORS, no key, actively maintained
    ...
    // API unreachable – show a curated local quote instead
    showLocalQuote();
  }
}
