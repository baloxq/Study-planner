...
async function loadQuote() {
  ...
  try {
    // Quotable API — open CORS, no proxy or API key needed
    const res = await fetch('https://api.quotable.io/random?tags=education|inspirational|wisdom');
    if (!res.ok) throw new Error('HTTP ' + res.status);
    const data = await res.json(); // returns { content, author }
    box.innerHTML = `
